...
Fabric `AppBackend` items are backend artifacts and may appear visually blank in the Fabric UX. The publish response also includes `widgetDashboardUrl`, which points to a visible companion KQL dashboard. That dashboard is currently manifest-aligned with placeholder KQL widgets (`Transaction count`, `Transaction count by Location`, `Agent summary`) until the semantic-model-to-Reyfin visual adapter is implemented.
